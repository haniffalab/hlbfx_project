...
if [ ! -d "$backup_dir" ]; then
  error "Directory $backup_dir does not exists in $BACKUP_BASE"
  backup_dir_present=1
fi

nobackup_dir_present=0
if [ ! -d "$nobackup_dir" ]; then
  error "Directory $nobackup_dir does not exists in $NOBACKUP_BASE"
...
if [ $backup_dir_present -eq 1 ] || [ $nobackup_dir_present -eq 1 ] || [ $user_dir_present -eq 1 ]; then
  error "Project cannot be added"
  error "Contact: bioinfo (hlbfx) team"
  exit 0
fi
